...
import { popularCalculators, liveCalculators } from "../lib/calculators";
...
export default function SearchBar({ onClose }: SearchBarProps) {
  ...
  const results = useMemo(() => {
    ...
    return liveCalculators.filter((calculator) =>
      `${calculator.title} ${calculator.description} ${calculator.keywords.join(" ")}`
        .toLowerCase()
        .includes(search)
    );
  }, [query]);
        ...
        <div className="mt-4">
          ...
          {results.length > 0 ? (
            ...
            <div className="rounded-2xl bg-slate-50 px-5 py-8 text-center">
              ...
              <h3 className="mt-3 font-bold text-slate-800">No calculator found</h3>
