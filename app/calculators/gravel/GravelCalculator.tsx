...
export default function GravelCalculator() {
  ...
  const [gravelType, setGravelType] = useState("Crushed Stone");
  ...
  const result = useMemo(() => {
    ...
    const volumeCuM = volumeCuFt * 0.0283168466;

    const wasteMultiplier = 1 + Math.max(0, wastePct || 0) / 100;
    ...
    const exactTons = volumeCuYd * densityValue;
    const orderTons = orderCuYd * densityValue;
  ...
  function handleGravelChange(value: string) {
    ...
    const selected = GRAVEL_TYPES.find(
      (item) => item.name === value
    );

    if (selected) {
      setDensity(String(selected.density));
    }
  }
    ...
    <section className="w-full">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 shadow-xl">
        <div className="px-5 py-5 sm:px-7">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-300">
            Estimate gravel volume, cubic yards, tons, waste and
            material cost for your project.
          </p>
        </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <select
                    value={gravelType}
                    onChange={(e) =>
                      handleGravelChange(e.target.value)
                    }
        ...
        >
          <div className="bg-slate-950 px-5 py-5 text-white sm:px-7">
            ...
            <p className="mt-2 text-sm text-slate-400">
              About {formatNumber(result.orderCuYd)} cubic yards
          ...
          <div className="grid grid-cols-2 gap-3 bg-slate-50 p-4 sm:grid-cols-4 sm:p-5">
            <ResultBox
              label="Cubic Yards"
              value={formatNumber(result.orderCuYd)}
              unit="yd³"
            />

            <ResultBox
              label="Cubic Feet"
              value={formatNumber(result.orderCuFt)}
              unit="ft³"
            />

            <ResultBox
              label="Cubic Meters"
              value={formatNumber(result.orderCuM)}
              unit="m³"
            />

            <ResultBox
              label="Area"
              value={formatNumber(result.areaSqFt)}
              unit="ft²"
            />
          </div>
          ...
          <div className="border-t border-slate-200 p-5 sm:p-7">
            ...
            <div className="mt-4 space-y-3 text-sm">
              ...
              <BreakdownRow
                ...
                value={`${formatNumber(result.wasteTons)} tons`}
              />

              <BreakdownRow
                label="Recommended order"
                value={`${formatNumber(result.orderTons)} tons`}
                strong
              />
              ...
              {result.cost !== null && (
                <BreakdownRow
                  ...
                  value={`${CURRENCIES[currency]}${formatNumber(
                    result.cost
                  )}`}
                  strong
                />
          ...
          <div className="border-t border-slate-200 bg-slate-50 px-5 py-4 text-xs leading-5 text-slate-500 sm:px-7">
            Gravel density varies by material, moisture and
            compaction. Use the density supplied by your local
            gravel supplier for a more accurate tonnage estimate.
          </div>
        </div>
      )}
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        ...
        <div className="mt-4 space-y-3 text-sm leading-7 text-slate-600">
          <p>
            <strong>Volume (cubic feet)</strong> = Length × Width ×
            Depth
          </p>

          <p>
            <strong>Cubic yards</strong> = Cubic feet ÷ 27
          </p>

          <p>
            <strong>Weight (tons)</strong> = Cubic yards × Gravel
            density
          </p>

          <p>
            <strong>Order quantity</strong> = Exact quantity ×
            (1 + waste ÷ 100)
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 rounded-2xl border border-blue-100 bg-blue-50 p-5 sm:p-7">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">
          For a 20 ft × 10 ft area with a 4-inch depth, the
          required volume is about 2.47 cubic yards. With 10%
          allowance and a density of 1.4 tons per cubic yard,
          the recommended order is about 3.81 tons.
        </p>
      </div>
      ...
      <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900">
        ...
        estimate. Actual gravel requirements can vary because of
        compaction, moisture, grading and the specific material
        used. Confirm final quantities with your supplier.
      </div>
    </section>
  );
}
